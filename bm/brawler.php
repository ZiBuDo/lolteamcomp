				...
				<div class="container">
					<div class="page-mainbar resume">
						...
						if(!isset($_GET['page']) || $_GET['page'] == 1){ //only do it on first page
							...
							if($_GET['region'] == "all"){
								$sql = "SELECT * FROM brawlers WHERE c1 = $_GET[id] OR c2 = $_GET[id] OR c3 = $_GET[id] OR c4 = $_GET[id] OR c5 = $_GET[id]"; //find all of this champs comps
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}else{
								$win_name = $_GET['region'] . "w";
								$loss_name = $_GET['region'] . "l"; //for rows in table
								$sql = "SELECT c1, c2, c3, c4, c5, $win_name, $loss_name FROM brawlers WHERE (c1 = $_GET[id] OR c2 = $_GET[id] OR c3 = $_GET[id] OR c4 = $_GET[id] OR c5 = $_GET[id]) AND ($win_name > 0 OR $loss_name > 0) "; //find all of this champs comps
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}
						...
						<table>
						<tr>
							<td><h1>Check Upgrades &nbsp &nbsp </h1></td>
							<td>
								<a href="<?php echo "http://www.lolteamcomp.com/bm/brawlerupgrade.php?champ=$_GET[champ]&id=$_GET[id]&region=all"?>" class="btn btn-info">Go </a>&nbsp;
							</td>
						</tr>
						</table>
							<br>
...
b.loading{
	...
	font-size: 200%;
											...
											$conn = mysqli_connect($servername, $username, $password, $dbname);
											if($_GET['region'] == "all"){
												$sql = "SELECT * FROM brawlers WHERE c1 = $_GET[id] OR c2 = $_GET[id] OR c3 = $_GET[id] OR c4 = $_GET[id] OR c5 = $_GET[id] ORDER BY ((naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw)/(naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw + nal + brl + trl+ rul+ ocel+ euwl+ eunel+ krl+ lasl+ lanl)) DESC LIMIT $page_get, 100"; //find all of this champs comps
												$result = mysqli_query($conn, $sql);
												$num = mysqli_num_rows($result);
											}else{
												$win_name = $_GET['region'] . "w";
												$loss_name = $_GET['region'] . "l"; //for rows in table
												$sql = "SELECT c1, c2, c3, c4, c5, $win_name, $loss_name FROM brawlers WHERE (c1 = $_GET[id] OR c2 = $_GET[id] OR c3 = $_GET[id] OR c4 = $_GET[id] OR c5 = $_GET[id]) AND ($win_name > 0 OR $loss_name > 0)  ORDER BY ($win_name/($loss_name + $win_name)) DESC LIMIT $page_get, 100"; //find all of this champs comps
												$result = mysqli_query($conn, $sql);
												$num = mysqli_num_rows($result);
											}
											?>
										...
										<tbody>
											...
											if (mysqli_num_rows($result) > 0) {
													...
													while($row = mysqli_fetch_assoc($result)) {
														...
														$id4 = $row["c4"];
														$id5 = $row["c5"];
														$ahref = "http://www.lolteamcomp.com/bm/brawlercomps.php?c1=$c1t&id1=$id1&c2=$c2t&id2=$id2&c3=$c3t&id3=$id3&c4=$c4t&id4=$id4&c5=$c5t&id5=$id5&region=all";//var for linking to specific comp
