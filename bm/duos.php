				...
				<div class="container">
					<div class="page-mainbar resume">
						...
						<div class="resume-header">
							<h1>Black Market Brawler Duo Compositions</h1>
						</div>
						<br>
						<?php
						if(!isset($_GET['page']) || $_GET['page'] == 1){ //only do it on first page
							...
							if($_GET['region'] == "all"){
								$sql = "SELECT * FROM duos "; //find all of the comps
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}else{
								$win_name = $_GET['region'] . "w";
								$loss_name = $_GET['region'] . "l"; //for rows in table
								$sql = "SELECT c1, c2,  $win_name, $loss_name FROM duos WHERE ($win_name > 0 OR $loss_name > 0)"; //find all of the comps of this region
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}
						?>
						<h2 style="text-align: center;"><?php echo $num;?> Unique Duo Compositions</h2>
							<?php } ?>
						<br>
						<?php 
...
b.loading{
	...
	font-size: 200%;
											...
											$conn = mysqli_connect($servername, $username, $password, $dbname);
											if($_GET['region'] == "all"){
												$sql = "SELECT * FROM duos ORDER BY ((naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw)/(naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw + nal + brl + trl+ rul+ ocel+ euwl+ eunel+ krl+ lasl+ lanl)) DESC LIMIT $page_get, 100"; //find all of the comps
												$result = mysqli_query($conn, $sql);
												$num = mysqli_num_rows($result);
											}else{
												$win_name = $_GET['region'] . "w";
												$loss_name = $_GET['region'] . "l"; //for rows in table
												$sql = "SELECT c1, c2, $win_name, $loss_name FROM duos WHERE ($win_name > 0 OR $loss_name > 0) ORDER BY ($win_name/($loss_name + $win_name)) DESC LIMIT $page_get, 100"; //find all of the comps of this region
												$result = mysqli_query($conn, $sql);
												$num = mysqli_num_rows($result);
											}
											?>
