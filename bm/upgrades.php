				...
				<div class="container">
					<div class="page-mainbar resume">
						...
						<div class="resume-header">
							<h1>Black Market Brawler Upgrades</h1>
						</div>
						<br>
						<?php
						if(!isset($_GET['page']) || $_GET['page'] == 1){
							...
							if($_GET['region'] == "all"){
								$sql = "SELECT * FROM upgrades"; //find all of the comps
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}else{
								$win_name = $_GET['region'] . "w";
								$loss_name = $_GET['region'] . "l"; //for rows in table
								$sql = "SELECT c1, c2, c3, c4, c5, c6, c7, c8, c9, c10,$win_name, $loss_name FROM upgrades WHERE ($win_name > 0 OR $loss_name > 0)"; //find all of the comps of this region
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}
						?>
...
b.loading{
	...
	font-size: 200%;
											...
											$conn = mysqli_connect($servername, $username, $password, $dbname);
											if($_GET['region'] == "all"){
												$sql = "SELECT * FROM upgrades ORDER BY ((naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw)/(naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw + nal + brl + trl+ rul+ ocel+ euwl+ eunel+ krl+ lasl+ lanl)) DESC LIMIT $page_get, 100"; //find all of the comps
												$result = mysqli_query($conn, $sql);
												$num = mysqli_num_rows($result);
											}else{
												$win_name = $_GET['region'] . "w";
												$loss_name = $_GET['region'] . "l"; //for rows in table
												$sql = "SELECT c1, c2, c3, c4, c5, c6, c7, c8, c9, c10,$win_name, $loss_name FROM upgrades WHERE ($win_name > 0 OR $loss_name > 0) ORDER BY ($win_name/($loss_name + $win_name)) DESC  LIMIT $page_get, 100"; //find all of the comps of this region
												$result = mysqli_query($conn, $sql);
												$num = mysqli_num_rows($result);
											}
											?>
										...
										<tbody>
											...
											if (mysqli_num_rows($result) > 0) {
													...
													while($row = mysqli_fetch_assoc($result)) {
														...
														$id9 = $row["c9"];
														$id10 = $row["c10"];
														$ahref = "http://www.lolteamcomp.com/bm/brawlerupgrade.php?champ=$c1t&id=$id1&region=all";//var for linking to specific comp
														...
														//makes sure that it isn't an all 0 composition
															...
															echo "<tr><td><a href=\"$ahref\"> <table><tr><td>$c6i $c7i $c8i $c9i $c10i</td></tr><tr>$c1i $c2i $c3i $c4i $c5i<td></td></tr></table> </a></td><td>$games</td><td>$wins</td><td>$losses</td><td>$wr</td></tr>"; //create a row of the table
