				...
				<div class="container">
					<div class="page-mainbar resume">
						...
						<div class="resume-header">
							<h1>Black Market Brawler Compositions</h1>
						</div>
						<br>
						<?php
							...
							if($_GET['region'] == "all"){
								$sql = "SELECT * FROM brawlers"; //find all of the comps
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}else{
								$win_name = $_GET['region'] . "w";
								$loss_name = $_GET['region'] . "l"; //for rows in table
								$sql = "SELECT c1, c2, c3, c4, c5, $win_name, $loss_name FROM brawlers WHERE ($win_name > 0 OR $loss_name > 0)"; //find all of the comps of this region
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}
						?>
...
b.loading{
	...
	font-size: 200%;
											...
											$conn = mysqli_connect($servername, $username, $password, $dbname);
											if($_GET['region'] == "all"){
												$sql = "SELECT * FROM brawlers ORDER BY ((naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw)/(naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw + nal + brl + trl+ rul+ ocel+ euwl+ eunel+ krl+ lasl+ lanl)) DESC"; //find all of the comps
												$result = mysqli_query($conn, $sql);
												$num = mysqli_num_rows($result);
											}else{
												$win_name = $_GET['region'] . "w";
												$loss_name = $_GET['region'] . "l"; //for rows in table
												$sql = "SELECT c1, c2, c3, c4, c5, $win_name, $loss_name FROM brawlers WHERE ($win_name > 0 OR $loss_name > 0) ORDER BY ($win_name/($loss_name + $win_name)) DESC"; //find all of the comps of this region
												$result = mysqli_query($conn, $sql);
												$num = mysqli_num_rows($result);
											}
											?>
