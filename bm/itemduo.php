				...
				<div class="container">
					<div class="page-mainbar resume">
						...
						<div class="resume-header" style="display:inline-block;">
							<!-- Image -->
							<?php 
							$result = file_get_contents('http://www.lolteamcomp.com/bm/items.txt');
							...
							$champ_name1 = $_GET['c1'];
							$id1 = $_GET['id1'];
							$href = "http://www.lolteamcomp.com/bm/item.php?champ=$champ_name&id=$id1&region=all";
							?>
						...
						<div class="resume-header" style="display:inline-block;">
							...
							$champ_name2 = $_GET['c2'];
							$id2 = $_GET['id2'];
							$href = "http://www.lolteamcomp.com/bm/item.php?champ=$champ_name&id=$id2&region=all";?>
							<a href=<?php echo $href; ?> > <img src="<?php echo "img/$key.png";?>" class="img-responsive" alt="" /></a>
							<!-- Heading -->
						...
						<?php }else{$champ_name2 = "none";} ?>
						<?php
						 if($_GET['c1'] != "none" && $_GET['c2'] != "none"){
						if(!isset($_GET['page']) || $_GET['page'] == 1){ //only do it on first page
							...
							$id_array = array($id1,$id2);
							sort($id_array);

							// Create connection
							$conn = mysqli_connect($servername, $username, $password, $dbname);
							if($_GET['region'] == "all"){
								$sql = "SELECT * FROM items WHERE (c1 = $id_array[0] AND c2 = $id_array[1])"; //find all of this champs comps with duo partner using all combos b/w 1-5
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}else{
								$win_name = $_GET['region'] . "w";
								$loss_name = $_GET['region'] . "l"; //for rows in table
								$sql = "SELECT c1, c2, $win_name, $loss_name FROM items WHERE (c1 = $id_array[0] AND c2 = $id_array[1]) AND ($win_name > 0 OR $loss_name > 0)"; //find all of this champs comps
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}
							$win_arr = array(); //add all wins here
							$loss_arr = array(); //add all losses here
							$win_num = 0;
							$loss_num = 0;
							if (mysqli_num_rows($result) > 0) {
								...
								foreach($win_arr as $win){
									$win_num = $win_num + $win;
								}
								foreach($loss_arr as $loss){
									$loss_num = $loss_num + $loss;
								}
								//calculate Winrate
								$winrate = number_format($win_num / ($win_num + $loss_num), 4, '.', ',');
								$winrate = $winrate * 100;
								$winrate = $winrate . "%"; //create percent
							}
						?>
						<h2 style="text-align: center;"><?php $games = $win_num + $loss_num; echo $games;?> Games Played</h2>
						<h2 style="text-align: center;"><?php echo $num;?> Unique Item Combos</h2>
						<h2 style="text-align: center;"><?php echo $winrate;?> Overall Winrate</h2>
						 <?php } }?>
						</div>
						<br>
						<?php 
						 if($_GET['c1'] != "none" && $_GET['c2'] != "none"){
							...
							?>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?c1=$champ_name1&id1=$id1&c2=$champ_name2&id2=$id2&region=$_GET[region]&page=$prev"?>" class="btn btn-info">Previous Page</a>&nbsp;
							</td>
							<?php } ?>
							<?php 
								if(isset($_GET['page'])){
							...
							?>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?c1=$champ_name1&id1=$id1&c2=$champ_name2&id2=$id2&region=$_GET[region]&page=$next"?>" class="btn btn-info">Next Page</a>&nbsp;
							</td>
						 <?php } ?>
							<br>
						<table>
						<tr>
							...
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?c1=$champ_name1&id1=$id1&c2=$champ_name2&id2=$id2&region=all"?>" class="btn btn-info">All </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?c1=$champ_name1&id1=$id1&c2=$champ_name2&id2=$id2&region=br"?>" class="btn btn-info">BR </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?c1=$champ_name1&id1=$id1&c2=$champ_name2&id2=$id2&region=eune"?>" class="btn btn-info">EUNE </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?c1=$champ_name1&id1=$id1&c2=$champ_name2&id2=$id2&region=euw"?>" class="btn btn-info">EUW </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?c1=$champ_name1&id1=$id1&c2=$champ_name2&id2=$id2&region=kr"?>" class="btn btn-info">KR </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?c1=$champ_name1&id1=$id1&c2=$champ_name2&id2=$id2&region=lan"?>" class="btn btn-info">LAN </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?c1=$champ_name1&id1=$id1&c2=$champ_name2&id2=$id2&region=las"?>" class="btn btn-info">LAS </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?c1=$champ_name1&id1=$id1&c2=$champ_name2&id2=$id2&region=na"?>" class="btn btn-info">NA </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?c1=$champ_name1&id1=$id1&c2=$champ_name2&id2=$id2&region=oce"?>" class="btn btn-info">OCE </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?c1=$champ_name1&id1=$id1&c2=$champ_name2&id2=$id2&region=ru"?>" class="btn btn-info">RU </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?c1=$champ_name1&id1=$id1&c2=$champ_name2&id2=$id2&region=tr"?>" class="btn btn-info">TR </a>&nbsp;
							</td>
						</tr>
						</table>
						<div class="resume-content">
							...
							<div class="row">
								<div class="col-md-2 col-sm-4">
									<!-- Heading -->
									<h3><i class="fa fa-angle-right"></i>Item Combos</h3>
								</div>
	...
	font-size: 200%;
											...
											//sort ids in numeric order so we can find the comp because all comps are sorts numerically ascendingly
											 if($_GET['c1'] != "none" && $_GET['c2'] != "none"){
												...
												$conn = mysqli_connect($servername, $username, $password, $dbname);
												if($_GET['region'] == "all"){
													$sql = "SELECT * FROM items WHERE (c1 = $id_array[0] AND c2 = $id_array[1]) ORDER BY ((naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw)/(naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw + nal + brl + trl+ rul+ ocel+ euwl+ eunel+ krl+ lasl+ lanl)) DESC LIMIT $page_get, 100"; //find all of this champs comps with duo partner using all combos b/w 1-5
													$result = mysqli_query($conn, $sql);
													$num = mysqli_num_rows($result);
												}else{
													$win_name = $_GET['region'] . "w";
													$loss_name = $_GET['region'] . "l"; //for rows in table
													$sql = "SELECT c1, c2, $win_name, $loss_name FROM items WHERE (c1 = $id_array[0] AND c2 = $id_array[1]) AND ($win_name > 0 OR $loss_name > 0)  ORDER BY ($win_name/($loss_name + $win_name)) DESC LIMIT $page_get, 100"; //find all of this champs comps
													$result = mysqli_query($conn, $sql);
													$num = mysqli_num_rows($result);
												}
											 }elseif($_GET['c1'] == "none"){
												 ...
												 $conn = mysqli_connect($servername, $username, $password, $dbname);
												if($_GET['region'] == "all"){
													$sql = "SELECT * FROM items WHERE c2 = $id_array[0] OR c1 = $id_array[0] ORDER BY ((naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw)/(naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw + nal + brl + trl+ rul+ ocel+ euwl+ eunel+ krl+ lasl+ lanl)) DESC"; //find all of this champs comps with duo partner only doubles this time
													$result = mysqli_query($conn, $sql);
													$num = mysqli_num_rows($result);
												}else{
													$win_name = $_GET['region'] . "w";
													$loss_name = $_GET['region'] . "l"; //for rows in table
													$sql = "SELECT c1, c2, $win_name, $loss_name FROM items WHERE (c2 = $id_array[0] OR c1 = $id_array[0]) AND ($win_name > 0 OR $loss_name > 0) ORDER BY ($win_name/($loss_name + $win_name)) DESC"; //find all of this champs comps
													$result = mysqli_query($conn, $sql);
													$num = mysqli_num_rows($result);
												}
											 }else{
											...
											?>
											<?php if($_GET['c1'] != "none" && $_GET['c2'] != "none"){ ?>
											...
											if($_GET['c1'] != "none" && $_GET['c2'] != "none"){
												if (mysqli_num_rows($result) > 0) {
														...
														while($row = mysqli_fetch_assoc($result)) {
															$wins = $row["naw"] + $row["euww"] + $row["brw"] + $row["eunew"] + $row["ocew"] + $row["lasw"] + $row["lanw"] + $row["krw"] + $row["trw"] + $row["ruw"]; //get all wins
														$losses = $row["nal"] + $row["euwl"] + $row["brl"] + $row["eunel"] + $row["ocel"] + $row["lasl"] + $row["lanl"] + $row["krl"] + $row["trl"] + $row["rul"]; //get all losses
														$ret = file_get_contents("http://www.lolteamcomp.com/bm/items.txt");
														$champs = json_decode($ret);
														if($row["c1"] == 1341 || $row["c1"] == 1340 || $row["c1"] == 1339 || $row["c1"] == 1338 || $row["c1"] == 1337 || $row["c1"] == 1336 || $row["c1"] == 1335){
															$rc1 = 3245;
														}else{
															$rc1 = $row["c1"]; //this allows us to set teleport boots and all ids to one id or if itsnt a teleport boot then we just continue
														}
														if($row["c2"] == 1341 || $row["c2"] == 1340 || $row["c2"] == 1339 || $row["c2"] == 1338 || $row["c2"] == 1337 || $row["c2"] == 1336 || $row["c2"] == 1335){
															$rc2 = 3245;
														}else{
															$rc2 = $row["c2"]; //this allows us to set teleport boots and all ids to one id or if itsnt a teleport boot then we just continue
														}
														$c1 = $champs->$rc1->key;
														$c2 = $champs->$rc2->key;
														...
														echo "<tr><td> $c1i $c2i</td><td>$games</td><td>$wins</td><td>$losses</td><td>$wr</td></tr>"; //create a row of the table
													}
												...
												if (mysqli_num_rows($result) > 0) {
														...
														$ahref = "http://www.lolteamcomp.com/bm/itemduo.php?c1=$c1t&id1=$id1&c2=$c2t&id2=$id2&region=all";//var for linking to specific comp
														...
														echo "<tr><td><a href=\"$ahref\"> $c1i $c2i</a></td><td>$games</td><td>$wins</td><td>$losses</td><td>$wr</td></tr>"; //create a row of the table
													}
														
													}else{
														echo "<tr><td>N/A (No games played for this champ)</td><td>0</td><td>0</td><td>0</td><td>0</td></tr>"; //no team comps
													}
											}
