				...
				<div class="container">
					<div class="page-mainbar resume">
						...
						<div class="resume-header" style="display:inline-block;">
							...
							$champ_name1 = $_GET['c1'];
							$id1 = $_GET['id1'];
							$href = "http://www.lolteamcomp.com/bm/champion.php?champ=$champ_name&id=$id1&region=all";
							?>
							<a href="<?php echo $href; ?>"> <img src="<?php echo "http://ddragon.leagueoflegends.com/cdn/5.16.1/img/champion/$key.png";?>" class="img-responsive" alt="" /></a>
							<!-- Heading -->
						...
						<div class="resume-header" style="display:inline-block;">
							...
							$champ_name2 = $_GET['c2'];
							$id2 = $_GET['id2'];
							$href = "http://www.lolteamcomp.com/bm/champion.php?champ=$champ_name&id=$id2&region=all";?>
							<a href=<?php echo $href; ?> > <img src="<?php echo "http://ddragon.leagueoflegends.com/cdn/5.16.1/img/champion/$key.png";?>" class="img-responsive" alt="" /></a>
							<!-- Heading -->
						...
						if(!isset($_GET['page']) || $_GET['page'] == 1){ //only do it on first page
							...
							if($_GET['region'] == "all"){
								$sql = "SELECT * FROM teams WHERE (c1 = $id_array[0] AND c2 = $id_array[1]) OR (c1 = $id_array[0] AND c3 = $id_array[1]) OR (c1 = $id_array[0] AND c4 = $id_array[1]) OR (c1 = $id_array[0] AND c5 = $id_array[1]) OR (c2 = $id_array[0] AND c3 = $id_array[1]) OR (c2 = $id_array[0] AND c4 = $id_array[1]) OR (c2 = $id_array[0] AND c5 = $id_array[1]) OR (c3 = $id_array[0] AND c4 = $id_array[1]) OR (c3 = $id_array[0] AND c5 = $id_array[1]) OR (c4 = $id_array[0] AND c5 = $id_array[1])"; //find all of this champs comps with duo partner using all combos b/w 1-5
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}else{
								$win_name = $_GET['region'] . "w";
								$loss_name = $_GET['region'] . "l"; //for rows in table
								$sql = "SELECT c1, c2, c3, c4, c5, $win_name, $loss_name FROM teams WHERE ((c1 = $id_array[0] AND c2 = $id_array[1]) OR (c1 = $id_array[0] AND c3 = $id_array[1]) OR (c1 = $id_array[0] AND c4 = $id_array[1]) OR (c1 = $id_array[0] AND c5 = $id_array[1]) OR (c2 = $id_array[0] AND c3 = $id_array[1]) OR (c2 = $id_array[0] AND c4 = $id_array[1]) OR (c2 = $id_array[0] AND c5 = $id_array[1]) OR (c3 = $id_array[0] AND c4 = $id_array[1]) OR (c3 = $id_array[0] AND c5 = $id_array[1]) OR (c4 = $id_array[0] AND c5 = $id_array[1])) AND ($win_name > 0 OR $loss_name > 0)"; //find all of this champs comps
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}
						...
						<div class="resume-content">
							...
							<div class="row">
								<div class="col-md-2 col-sm-4">
									<!-- Heading -->
									<h3><i class="fa fa-angle-right"></i>Duo Comps</h3>
								</div>
	...
	font-size: 200%;
											...
											//sort ids in numeric order so we can find the comp because all comps are sorts numerically ascendingly
											 if($_GET['c1'] != "none" && $_GET['c2'] != "none"){
												...
												$conn = mysqli_connect($servername, $username, $password, $dbname);
												if($_GET['region'] == "all"){
													$sql = "SELECT * FROM teams WHERE (c1 = $id_array[0] AND c2 = $id_array[1]) OR (c1 = $id_array[0] AND c3 = $id_array[1]) OR (c1 = $id_array[0] AND c4 = $id_array[1]) OR (c1 = $id_array[0] AND c5 = $id_array[1]) OR (c2 = $id_array[0] AND c3 = $id_array[1]) OR (c2 = $id_array[0] AND c4 = $id_array[1]) OR (c2 = $id_array[0] AND c5 = $id_array[1]) OR (c3 = $id_array[0] AND c4 = $id_array[1]) OR (c3 = $id_array[0] AND c5 = $id_array[1]) OR (c4 = $id_array[0] AND c5 = $id_array[1]) ORDER BY ((naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw)/(naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw + nal + brl + trl+ rul+ ocel+ euwl+ eunel+ krl+ lasl+ lanl)) DESC LIMIT $page_get, 100"; //find all of this champs comps with duo partner using all combos b/w 1-5
													$result = mysqli_query($conn, $sql);
													$num = mysqli_num_rows($result);
												}else{
													$win_name = $_GET['region'] . "w";
													$loss_name = $_GET['region'] . "l"; //for rows in table
													$sql = "SELECT c1, c2, c3, c4, c5, $win_name, $loss_name FROM teams WHERE ((c1 = $id_array[0] AND c2 = $id_array[1]) OR (c1 = $id_array[0] AND c3 = $id_array[1]) OR (c1 = $id_array[0] AND c4 = $id_array[1]) OR (c1 = $id_array[0] AND c5 = $id_array[1]) OR (c2 = $id_array[0] AND c3 = $id_array[1]) OR (c2 = $id_array[0] AND c4 = $id_array[1]) OR (c2 = $id_array[0] AND c5 = $id_array[1]) OR (c3 = $id_array[0] AND c4 = $id_array[1]) OR (c3 = $id_array[0] AND c5 = $id_array[1]) OR (c4 = $id_array[0] AND c5 = $id_array[1])) AND ($win_name > 0 OR $loss_name > 0)  ORDER BY ($win_name/($loss_name + $win_name)) DESC LIMIT $page_get, 100"; //find all of this champs comps
													$result = mysqli_query($conn, $sql);
													$num = mysqli_num_rows($result);
												}
											 }elseif($_GET['c1'] == "none"){
												 ...
												 $conn = mysqli_connect($servername, $username, $password, $dbname);
												if($_GET['region'] == "all"){
													$sql = "SELECT * FROM duos WHERE c2 = $id_array[0] OR c1 = $id_array[0] ORDER BY ((naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw)/(naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw + nal + brl + trl+ rul+ ocel+ euwl+ eunel+ krl+ lasl+ lanl)) DESC"; //find all of this champs comps with duo partner only doubles this time
													$result = mysqli_query($conn, $sql);
													$num = mysqli_num_rows($result);
												}else{
													$win_name = $_GET['region'] . "w";
													$loss_name = $_GET['region'] . "l"; //for rows in table
													$sql = "SELECT c1, c2, $win_name, $loss_name FROM duos WHERE (c2 = $id_array[0] OR c1 = $id_array[0]) AND ($win_name > 0 OR $loss_name > 0) ORDER BY ($win_name/($loss_name + $win_name)) DESC"; //find all of this champs comps
													$result = mysqli_query($conn, $sql);
													$num = mysqli_num_rows($result);
												}
											 }else{
											...
											if($_GET['c1'] != "none" && $_GET['c2'] != "none"){
												...
												if (mysqli_num_rows($result) > 0) {
														...
														while($row = mysqli_fetch_assoc($result)) {
															...
															$ahref = "http://www.lolteamcomp.com/bm/duo.php?c1=$c1t&id1=$id1&c2=$c2t&id2=$id2&region=all";//var for linking to specific comp
															$c1i = "<img title=$c1t height=32 width=32 src='http://ddragon.leagueoflegends.com/cdn/5.16.1/img/champion/$c1.png'/>";
															$c2i = "<img title=$c2t height=32 width=32 src='http://ddragon.leagueoflegends.com/cdn/5.16.1/img/champion/$c2.png'/>";
															$c3i = "<img title=$c3t height=32 width=32 src='http://ddragon.leagueoflegends.com/cdn/5.16.1/img/champion/$c3.png'/>";
															$c4i = "<img title=$c4t height=32 width=32 src='http://ddragon.leagueoflegends.com/cdn/5.16.1/img/champion/$c4.png'/>";
