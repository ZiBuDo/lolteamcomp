				...
				<div class="container">
					<div class="page-mainbar resume">
						...
						<div class="resume-header">
							<h1>Black Market Brawler Item Compositions</h1>
						</div>
						<br>
						<?php
						if(!isset($_GET['page']) || $_GET['page'] == 1){ //only do it on first page
							...
							if($_GET['region'] == "all"){
								$sql = "SELECT * FROM items"; //find all of the comps
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}else{
								$win_name = $_GET['region'] . "w";
								$loss_name = $_GET['region'] . "l"; //for rows in table
								$sql = "SELECT c1, c2,  $win_name, $loss_name FROM items WHERE ($win_name > 0 OR $loss_name > 0)"; //find all of the comps of this region
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}
						?>
...
b.loading{
	...
	font-size: 200%;
											...
											$conn = mysqli_connect($servername, $username, $password, $dbname);
											if($_GET['region'] == "all"){
												$sql = "SELECT * FROM items ORDER BY ((naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw)/(naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw + nal + brl + trl+ rul+ ocel+ euwl+ eunel+ krl+ lasl+ lanl)) DESC LIMIT $page_get, 100"; //find all of the comps
												$result = mysqli_query($conn, $sql);
												$num = mysqli_num_rows($result);
											}else{
												$win_name = $_GET['region'] . "w";
												$loss_name = $_GET['region'] . "l"; //for rows in table
												$sql = "SELECT c1, c2, $win_name, $loss_name FROM items WHERE ($win_name > 0 OR $loss_name > 0) ORDER BY ($win_name/($loss_name + $win_name)) DESC LIMIT $page_get, 100"; //find all of the comps of this region
												$result = mysqli_query($conn, $sql);
												$num = mysqli_num_rows($result);
											}
											?>
										...
										<tbody>
											...
											if (mysqli_num_rows($result) > 0) {
													...
													while($row = mysqli_fetch_assoc($result)) {
														...
														$ahref = "http://www.lolteamcomp.com/bm/item.php?c1=$c1t&id1=$id1&region=all";//var for linking to specific comp
													...
													echo "<tr><td>N/A (No comps left)</td><td>0</td><td>0</td><td>0</td><td>0</td></tr>"; //no team comps
												}
