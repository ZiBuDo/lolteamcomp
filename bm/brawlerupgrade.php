				...
				<div class="container">
					<div class="page-mainbar resume">
						...
						<div class="resume-header">
							...
							$title = $champs->$_GET['id']->title; //match through id and find the title and key
							$key = $champs->$_GET['id']->key;?>
							<img src="<?php echo "img/$key.png";?>" class="img-responsive" alt="" />
							<!-- Heading -->
							<h2><?php echo $_GET['champ'];?></h2>
							<!-- Paragraph -->
						...
						if(!isset($_GET['page']) || $_GET['page'] == 1){ //only do it on first page
							...
							if($_GET['region'] == "all"){
								$sql = "SELECT * FROM upgrades WHERE c1 = $key "; //find all of the comps
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}else{
								$win_name = $_GET['region'] . "w";
								$loss_name = $_GET['region'] . "l"; //for rows in table
								$sql = "SELECT c1, c2, c3, c4, c5, c6, c7, c8, c9, c10,$win_name, $loss_name FROM upgrades WHERE ($win_name > 0 OR $loss_name > 0) AND c1 = $key"; //find all of the comps of this region
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}
						...
						<?php 
							...
							?>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?champ=$_GET[champ]&id=$_GET[id]&region=$_GET[region]&page=$prev"?>" class="btn btn-info">Previous Page</a>&nbsp;
							</td>
							<?php } ?>
							<?php 
								if(isset($_GET['page'])){
							...
							?>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?champ=$_GET[champ]&id=$_GET[id]&region=$_GET[region]&page=$next"?>" class="btn btn-info">Next Page</a>&nbsp;
							</td>
							<br>
						<table>
						<tr>
							...
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?champ=$_GET[champ]&id=$_GET[id]&region=all"?>" class="btn btn-info">All </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?champ=$_GET[champ]&id=$_GET[id]&region=br"?>" class="btn btn-info">BR </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?champ=$_GET[champ]&id=$_GET[id]&region=eune"?>" class="btn btn-info">EUNE </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?champ=$_GET[champ]&id=$_GET[id]&region=euw"?>" class="btn btn-info">EUW </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?champ=$_GET[champ]&id=$_GET[id]&region=kr"?>" class="btn btn-info">KR </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?champ=$_GET[champ]&id=$_GET[id]&region=lan"?>" class="btn btn-info">LAN </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?champ=$_GET[champ]&id=$_GET[id]&region=las"?>" class="btn btn-info">LAS </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?champ=$_GET[champ]&id=$_GET[id]&region=na"?>" class="btn btn-info">NA </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?champ=$_GET[champ]&id=$_GET[id]&region=oce"?>" class="btn btn-info">OCE </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?champ=$_GET[champ]&id=$_GET[id]&region=ru"?>" class="btn btn-info">RU </a>&nbsp;
							</td>
							<td>
								<a href="<?php echo $_SERVER['PHP_SELF'] . "?champ=$_GET[champ]&id=$_GET[id]&region=tr"?>" class="btn btn-info">TR </a>&nbsp;
							</td>
						</tr>
						</table>
...
b.loading{
	...
	font-size: 200%;
											...
											$conn = mysqli_connect($servername, $username, $password, $dbname);
											if($_GET['region'] == "all"){
												$sql = "SELECT * FROM upgrades WHERE c1 = $key ORDER BY ((naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw)/(naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw + nal + brl + trl+ rul+ ocel+ euwl+ eunel+ krl+ lasl+ lanl)) DESC LIMIT $page_get, 100"; //find all of the comps
												$result = mysqli_query($conn, $sql);
												$num = mysqli_num_rows($result);
											}else{
												$win_name = $_GET['region'] . "w";
												$loss_name = $_GET['region'] . "l"; //for rows in table
												$sql = "SELECT c1, c2, c3, c4, c5, c6, c7, c8, c9, c10,$win_name, $loss_name FROM upgrades WHERE ($win_name > 0 OR $loss_name > 0) AND c1 = $key ORDER BY ($win_name/($loss_name + $win_name)) DESC  LIMIT $page_get, 100"; //find all of the comps of this region
												$result = mysqli_query($conn, $sql);
												$num = mysqli_num_rows($result);
											}
											?>
										...
										<tbody>
											...
											if (mysqli_num_rows($result) > 0) {
													...
													while($row = mysqli_fetch_assoc($result)) {
														...
														$champs = json_decode($ret); //actually brawlers in this context
														$c1 = $champs->$row["c1"]->key;
														$c2 = $champs->$row["c2"]->key;
														$c3 = $champs->$row["c3"]->key;
														$c4 = $champs->$row["c4"]->key;
														$c5 = $champs->$row["c5"]->key;
														$c6 = $champs->$row["c6"]->key;
														$c7 = $champs->$row["c7"]->key;
														$c8 = $champs->$row["c8"]->key;
														$c9 = $champs->$row["c9"]->key;
														$c10 = $champs->$row["c10"]->key;
														$c1t = $champs->$row["c1"]->name;
														$c2t = $champs->$row["c2"]->name;
														$c3t = $champs->$row["c3"]->name;
														$c4t = $champs->$row["c4"]->name;
														$c5t = $champs->$row["c5"]->name;
														$c6t = $champs->$row["c6"]->name;
														$c7t = $champs->$row["c7"]->name;
														$c8t = $champs->$row["c8"]->name;
														$c9t = $champs->$row["c9"]->name;
														$c10t = $champs->$row["c10"]->name;
														$id1 = $row["c1"];
														$id2 = $row["c2"];
														$id3 = $row["c3"];
														$id4 = $row["c4"];
														...
														$c6i = "<img title=$c6t height=32 width=32 src='img/$c6.png'/>";
														$c7i = "<img title=$c7t height=32 width=32 src='img/$c7.png'/>";
														$c8i = "<img title=$c8t height=32 width=32 src='img/$c8.png'/>";
														$c9i = "<img title=$c9t height=32 width=32 src='img/$c9.png'/>";
														$c10i = "<img title=$c10t height=32 width=32 src='img/$c10.png'/>";
														$games = $wins + $losses;
														$wr = number_format($wins / ($wins + $losses), 2, '.', ',');
														//makes sure that it isn't an all 0 composition
														echo "<tr><td> <table><tr><td>$c6i $c7i $c8i $c9i $c10i</td></tr><tr>$c1i $c2i $c3i $c4i $c5i<td></td></tr></table></td><td>$games</td><td>$wins</td><td>$losses</td><td>$wr</td></tr>"; //create a row of the table
													}
