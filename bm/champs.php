						...
						<div class="resume-content">
							...
							<div class="row">
								<div class="col-md-2 col-sm-4">
									<!-- Heading -->
									<h3><i class="fa fa-angle-right"></i>Champions</h3>
								</div>
...
b.loading{
	...
	font-size: 200%;
											...
											if($_GET['region'] == "all"){
												$sql = "SELECT * FROM champs ORDER BY ((naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw)/(naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw + nal + brl + trl+ rul+ ocel+ euwl+ eunel+ krl+ lasl+ lanl)) DESC"; //find all of the comps
												$result = mysqli_query($conn, $sql);
												$num = mysqli_num_rows($result);
											}else{
												$win_name = $_GET['region'] . "w";
												$loss_name = $_GET['region'] . "l"; //for rows in table
												$sql = "SELECT c1, $win_name, $loss_name FROM champs WHERE ($win_name > 0 OR $loss_name > 0) ORDER BY ($win_name/($loss_name + $win_name)) DESC"; //find all of the comps of this region
												$result = mysqli_query($conn, $sql);
												$num = mysqli_num_rows($result);
											}
											?>
										...
										<tbody>
											...
											if (mysqli_num_rows($result) > 0) {
													...
													while($row = mysqli_fetch_assoc($result)) {
														...
														$ahref = "http://www.lolteamcomp.com/bm/champion.php?champ=$c1t&id=$id1&region=all";//var for linking to specific comp
														...
														echo "<tr><td><a href=\"$ahref\"> $c1i </a></td><td>$games</td><td>$wins</td><td>$losses</td><td>$wr</td></tr>"; //create a row of the table
													}
