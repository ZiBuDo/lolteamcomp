				...
				<div class="container">
					<div class="page-mainbar resume">
						...
						<div class="resume-header">
							<!-- Image -->
							<?php $result = file_get_contents('http://www.lolteamcomp.com/bm/items.txt');
						...
						if(!isset($_GET['page']) || $_GET['page'] == 1){ //only do it on first page
							...
							if($_GET['region'] == "all"){
								$sql = "SELECT * FROM items WHERE c1 = $_GET[id] OR c2 = $_GET[id]"; //find all of this champs comps
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}else{
								$win_name = $_GET['region'] . "w";
								$loss_name = $_GET['region'] . "l"; //for rows in table
								$sql = "SELECT c1, c2, $win_name, $loss_name FROM items WHERE (c1 = $_GET[id] OR c2 = $_GET[id]) AND ($win_name > 0 OR $loss_name > 0) "; //find all of this champs comps
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}
						...
						<table>
						<tr>
							<td><h1>Check Recipe &nbsp &nbsp </h1></td>
							<td>
								<a href="<?php echo "http://www.lolteamcomp.com/bm/itemrecipe.php?champ=$_GET[champ]&id=$_GET[id]&region=all"?>" class="btn btn-info">Go </a>&nbsp;
							</td>
						</tr>
						</table>
							<br>
						...
						<div class="resume-content">
							...
							<div class="row">
								<div class="col-md-2 col-sm-4">
									<!-- Heading -->
									<h3><i class="fa fa-angle-right"></i>Item Comps</h3>
								</div>
...
b.loading{
	...
	font-size: 200%;
											...
											$conn = mysqli_connect($servername, $username, $password, $dbname);
											if($_GET['region'] == "all"){
												$sql = "SELECT * FROM items WHERE c1 = $_GET[id] OR c2 = $_GET[id] ORDER BY ((naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw)/(naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw + nal + brl + trl+ rul+ ocel+ euwl+ eunel+ krl+ lasl+ lanl)) DESC LIMIT $page_get, 100"; //find all of this champs comps
												$result = mysqli_query($conn, $sql);
												$num = mysqli_num_rows($result);
											}else{
												$win_name = $_GET['region'] . "w";
												$loss_name = $_GET['region'] . "l"; //for rows in table
												$sql = "SELECT c1, c2, $win_name, $loss_name FROM items WHERE (c1 = $_GET[id] OR c2 = $_GET[id]) AND ($win_name > 0 OR $loss_name > 0)  ORDER BY ($win_name/($loss_name + $win_name)) DESC LIMIT $page_get, 100"; //find all of this champs comps
												$result = mysqli_query($conn, $sql);
												$num = mysqli_num_rows($result);
											}
											?>
