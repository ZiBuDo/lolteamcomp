				...
				<div class="container">
					<div class="page-mainbar resume">
						...
						<div class="resume-header">
							...
							<img src="<?php echo "http://ddragon.leagueoflegends.com/cdn/5.16.1/img/champion/$key.png";?>" class="img-responsive" alt="" />
							<!-- Heading -->
						...
						if(!isset($_GET['page']) || $_GET['page'] == 1){ //only do it on first page
							...
							if($_GET['region'] == "all"){
								$sql = "SELECT * FROM teams WHERE c1 = $_GET[id] OR c2 = $_GET[id] OR c3 = $_GET[id] OR c4 = $_GET[id] OR c5 = $_GET[id] "; //find all of this champs comps
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}else{
								$win_name = $_GET['region'] . "w";
								$loss_name = $_GET['region'] . "l"; //for rows in table
								$sql = "SELECT c1, c2, c3, c4, c5, $win_name, $loss_name FROM teams WHERE (c1 = $_GET[id] OR c2 = $_GET[id] OR c3 = $_GET[id] OR c4 = $_GET[id] OR c5 = $_GET[id]) AND ($win_name > 0 OR $loss_name > 0)  "; //find all of this champs comps
								$result = mysqli_query($conn, $sql);
								$num = mysqli_num_rows($result);
							}
...
b.loading{
	...
	font-size: 200%;
											...
											$conn = mysqli_connect($servername, $username, $password, $dbname);
											if($_GET['region'] == "all"){
												$sql = "SELECT * FROM teams WHERE c1 = $_GET[id] OR c2 = $_GET[id] OR c3 = $_GET[id] OR c4 = $_GET[id] OR c5 = $_GET[id] ORDER BY ((naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw)/(naw + brw + trw+ ruw+ ocew+ euww+ eunew+ krw+ lasw+ lanw + nal + brl + trl+ rul+ ocel+ euwl+ eunel+ krl+ lasl+ lanl)) DESC"; //find all of this champs comps
												$result = mysqli_query($conn, $sql);
												$num = mysqli_num_rows($result);
											}else{
												$win_name = $_GET['region'] . "w";
												$loss_name = $_GET['region'] . "l"; //for rows in table
												$sql = "SELECT c1, c2, c3, c4, c5, $win_name, $loss_name FROM teams WHERE (c1 = $_GET[id] OR c2 = $_GET[id] OR c3 = $_GET[id] OR c4 = $_GET[id] OR c5 = $_GET[id]) AND ($win_name > 0 OR $loss_name > 0)  ORDER BY ($win_name/($loss_name + $win_name)) DESC"; //find all of this champs comps
												$result = mysqli_query($conn, $sql);
												$num = mysqli_num_rows($result);
											}
											?>
